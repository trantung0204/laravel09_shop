...
@section('content')
            ...
            <!-- /.box-header -->
            <div class="box-body">
              <table id="brand-table" data-url="{{route('getBrands')}}" class="table table-bordered table-striped">
                <thead>
                <tr>
                  <th>#</th>
                  <th>Name</th>
              ...
              <a class="btn btn-success" data-toggle="modal" href='#add' >Add new brand</a> 
            </div>
            <!-- /.box-body -->
          </div>
          <!-- /.box -->
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Add more brands</h4>
        </div>
        <div class="modal-body">
        <form id="add-new"  method="POST" role="form"  data-url='{{route("brands.store")}}' >
          {{csrf_field()}}<!-- sinh token -->
          <div class="form-group">
            <label for="name">Brand Name</label>
            <input name="name" type="text" id="name" class="form-control" placeholder="Brand name" required="true">
          </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form id="edit-form"  method="POST" role="form" >
          ...
          <div class="form-group">
            <label for="name">Brand Name</label>
            <input name="name" type="text" id="edit_name" class="form-control" placeholder="Brand name" required="true">
          </div>
...
<script src="{{ asset('admin_assets/js/ajax_brand.js') }}"></script>
{{-- <script>
$(function() {
    $('#brand-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route('getBrands') !!}',
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
